{
  ...
  in flake-utils.lib.eachSystem [
    ...
    let
      ...
      kxstsOverlay = final: prev:
      let
        kxsts-pyk = final.callPackage ./nix/kxsts-pyk {
          inherit pyproject-nix pyproject-build-systems uv2nix;
          python = final."python${pythonVer}";
        };
        kxsts = final.callPackage ./nix/kxsts {
          inherit kxsts-pyk;
    ...
    in {
      ...
      packages = rec {
        inherit (pkgs) kxsts uv;
        default = kxsts;
      };
    }) // {
      overlays.default = final: prev: {
        inherit (self.packages.${final.system}) kxsts;
      };
    };
}
