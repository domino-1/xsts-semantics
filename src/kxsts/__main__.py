...
def exec_run(
    ...
    **kwargs: Any,
    ...
    pattern = kxsts.pattern(pgm=pgm, env=env)
    output = kxsts.run(pattern, depth=depth)
    print(kxsts.pretty(output, color=True))
...
def exec_show(
    ...
    **kwargs: Any,
    ...
    kxsts = KXsts()
    kxsts.show_kcfg(spec_module, claim_id)
...
def exec_view(
    ...
    **kwargs: Any,
    ...
    kxsts = KXsts()
    kxsts.view_kcfg(spec_module, claim_id)
...
def create_argument_parser() -> ArgumentParser:
    ...
    parser = ArgumentParser(prog='kxsts', description='KXsts command line tool')
    ...
    run_subparser = command_parser.add_parser('run', help='Run an XSTS program', parents=[shared_args])
    run_subparser.add_argument('input_file', metavar='INPUT_FILE', type=file_path, help='Path to .xsts file')
